...
void ParticleFilter::prediction(double delta_t, double std_pos[], 
  ...
  for(auto& p: particles){
    ...
    if( fabs(yaw_rate) < ZERO){  // constant velocity
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
  ...
  for(auto& p: particles){
    ...
    vector<LandmarkObs> Map_landmarks_inRange;
    for(const auto& lm: map_landmarks.landmark_list){
      ...
      if( distance < sensor_range){ // if the landmark is within the sensor range, save it to predictions
        Map_landmarks_inRange.push_back(LandmarkObs{lm.id_i, lm.x_f, lm.y_f});
      }
    }
    
    // step 2: convert observations from vehicle coordinate to map coordinate.
    vector<LandmarkObs> observations_trans2map;
    ...
    for(const auto& obs: observations){
      ...
      observations_trans2map.push_back(tmp);
    }
    
    // step 3: find landmark index for each observation
    dataAssociation(Map_landmarks_inRange, observations_trans2map);
    
    // step 4: compute the particle's weight:
    for(const auto& obs_m: observations_trans2map){
